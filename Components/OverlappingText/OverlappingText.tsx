...
export default function OverlappingText({classname, toptext, backgroundtext}: props){
  ...
  return <div ref={ref} className={(classname?classname:"") + "relative flex items-center justify-center text-center py-12 my-12"}>
    <animated.div style={fadetop} className="text-end absolute font-bold text-md sm:text-3xl bottom-8 sm:bottom-16">
      {toptext}
    </animated.div>
    <animated.div style={fade} className="text-slate-700 font-black uppercase leading-6 text-5xl sm:text-8xl align-text-bottom tracking-widest">
      {backgroundtext}
    </animated.div>
  </div>
}